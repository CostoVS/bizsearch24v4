...
function SearchBarForm() {
  ...
  // Expanded groups state for category accordion inside popover
    ...
    >
      ...
      <div className="relative flex-[1.4] min-w-0">
        ...
        {openDropdown === 'category' && (
          <div className="absolute top-full left-0 right-0 md:left-auto md:right-0 mt-2 z-50 bg-white rounded-3xl border border-slate-200/90 shadow-2xl shadow-emerald-950/30 p-3.5 w-full md:w-[580px] lg:w-[680px] max-w-[calc(100vw-1.5rem)] animate-in fade-in-50 zoom-in-95 duration-150">
            
            {/* Header with Search and Full Categories Link */}
            <div className="flex items-center justify-between gap-3 mb-2.5 pb-2.5 border-b border-slate-100">
              <div className="flex items-center gap-2">
            ...
            <div className="relative mb-2.5">
              ...
              <input
                ...
                placeholder="Search main categories or specific trades (e.g. plumber, legal, salon)..."
                className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-8 pr-7 py-2 text-xs font-semibold text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition"
                autoFocus
              />
              {categorySearch && (
            ...
            <div className="max-h-[60vh] md:max-h-[460px] overflow-y-auto space-y-2 pr-1.5 text-xs [scrollbar-width:thin]">
              ...
              {/* Category Groups with Accordion & Subcategories */}
              {filteredCategoriesStructured.map((group) => {
                const isGroupSelected = isSubcategoryOf(category, group.name) && !group.items.some(it => it.fullName === category);
                const isExpanded = categorySearch ? true : (expandedGroups[group.name] ?? false);
                const icon = group.icon;

                return (
                  <div key={group.name} className="bg-slate-50/80 border border-slate-200/90 rounded-2xl overflow-hidden transition-all">
                    
                    {/* Header: Click parent name to select group or click toggle to expand subcategories */}
                    <div className="p-2 flex items-center justify-between gap-2">
                      <button
                        ...
                        className={`flex-1 text-left px-2.5 py-1.5 rounded-xl font-extrabold text-xs uppercase tracking-wide flex items-center justify-between gap-2 transition ${
                          category === group.name
                      ...
                      >
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md ${
                            category === group.name ? 'bg-white/20 text-white' : 'bg-slate-200/80 text-slate-600'
                          }`}>
                            {group.items.length}
                          </span>
                        </div>
                        {category === group.name && <Check className="w-3 h-3 text-white shrink-0" />}
                      </button>
                      ...
                      <button
                        ...
                        className="p-1.5 text-slate-500 hover:text-slate-800 hover:bg-slate-200/70 rounded-lg transition shrink-0"
                        title={isExpanded ? "Collapse subcategories" : "Show subcategories"}
                      >
                        <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${isExpanded ? 'rotate-180 text-emerald-700' : ''}`} />
                      </button>
                    </div>

                    {/* SUBCATEGORIES GRID */}
                    {isExpanded && (
                      <div className="px-3 pb-3 pt-1 border-t border-slate-200/60 bg-white grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                            ...
                            <button
                              ...
                              className={`w-full text-left px-2.5 py-1.5 rounded-lg text-xs font-medium flex items-center justify-between transition ${
                                isSubSelected
                                  ...
                                  : 'text-slate-700 hover:bg-emerald-50 hover:text-emerald-950 bg-slate-50/70'
                              }`}
                            >
                              <div className="flex items-center gap-1.5 min-w-0">
                                <span className={`font-mono text-[10px] font-bold px-1.5 py-0.5 rounded ${
                                  isSubSelected ? 'bg-white/25 text-white' : 'bg-emerald-100 text-emerald-800'
                                }`}>
                                  {item.id}
                                </span>
                                <span className="truncate">{item.name}</span>
                              </div>
                              {isSubSelected && <Check className="w-3 h-3 text-white flex-shrink-0 ml-1.5" />}
                            </button>
                          );
                        })}
                      </div>
